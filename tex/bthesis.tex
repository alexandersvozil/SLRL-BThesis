...
After thorougly discussing the decision problem of SLRL, the optimization problem
...
\begin{algorithm}[H]
  ...
  \end{algorithmic}
\end{algorithm}
The evaluation of this algorithm using the test instances can be seen on table 
\ref{table:greedyloc}. The runtime of the original algorithm is $\mathcal O(kn^{2}*(V+E))$,
...
To determine the shortest paths from one node to every other node the runtime
is $\mathcal O(V+E)$ using Breadth-First Search. This needs to be done for every node once,
which implies a runtime of $\mathcal O(n*(V+E))$. 
This calculations suffice to determine the neighbor sets. To determine the maximum
edge usage, every node (except the nodes in the server set) need to be looked 
at again (the usage of the edges in the
path(s) to the server(s) need to be incremented), 
