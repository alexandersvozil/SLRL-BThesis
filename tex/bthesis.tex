...
\For{i=0; i$<$iter; i++}
  ...
  \If{(curS.getLastC()$\leq$ c) $\&\&$ (curS.getR() $<$ bestS.getR()) }
    \State bestSolution.setSolved(true);
    \State bestSolution = currentSol;
    \If{curS.getR()==1}
      \State break; \Comment {Best possible feasible Solution}
    \EndIf
  \EndIf  
   \If{(!bestS.isSolved()) $\&\&$ (bestS.getLastC()$>$curS.getLastC()) }
...
The local search exchanges a server with every other node and 
recalculates the constraints to create a neighborhood N of solutions.
The process is repeated for every server in the given Set of Servers.
The algorithm chooses the solution by the best improvement strategy,
meaning it enumerates the whole neighborhood and then chooses the 
...
\For{Node server : g.getServers()}
  ...
  \For{Node otherNode: g.getGraph())}
    \If{(!otherNode.equals(server))\&\&(!g.getServers().contains(otherNode))}
      ...
      \If{(max\_usage$\leq$c)\&\&((max\_neighbourset)$<$bestSolution.getR())
      \&\&(!tabuList.contains(g.getServers()))}
        \State bestSolution=new Solution(g,max\_neighbourset,max\_usage,true);
      \EndIf 
      \If{(!bestSolution.isSolved())\&\&(bestSolution.getLastC()$>$max\_usage)
      \&\&(!tabuList.contains(g.getServers()))}
...
\Require Solution curS, Graph(V,E) g, MaxEdgeUsage c, Runtime $r_{sec}$, Temperature $t_{init}$, Cooling factor c,
        ...
        \If{compareSolutions(solX,solY)} \Comment If solY is better than solX
                \State solX = solY;
         \Else
        ...
        \EndIf    
            ...
            \If{(solX.getR()==r\_lower)\&\&(solX.getLastC() $\leq$ c)}
               \State break;
            \EndIf 
      \EndWhile\\
      \Return solX;
