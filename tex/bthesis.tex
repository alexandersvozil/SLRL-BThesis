...
could be an expensive choice, inparticular, if no qualifying calculation is done. %considering no prior calculation where it should go. 
...
In order to understand the definition of the SLRL decision problem, the notions of a neighbour set and the load on a link need to be introduced. 
\subsection{Neighbour Set}
Given a graph $G=(V,E)$ and a set of servers $S \subseteq V$, the neighbour set of a server is the set of vertices, that belong to a server $s \in S$. In the SLRL problem, it is
defined as follows: If a non-server vertex has a shortest path to a server node, it 
belongs to this server nodes neighbour set. If the vertex has several shortest
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    [scale=0.85,auto=left,every node/.style={scale=0.85}]
    \node[circle,draw, red] (a) at (0,3) {s1};
    \node[circle,draw] (b) at (1,4) {v1};
    \node[circle,draw] (c) at (1,2) {v2};
    \node[circle,draw] (d) at (2,3) {v3};
    \node[circle,draw] (e) at (4,3) {v4};
    \node[circle,draw] (f) at (6,3) {v5};
    \node[circle,draw] (g) at (8,3) {v6};
    \node[circle,draw, blue] (h) at (9,4) {s2};
    \node[circle,draw] (i) at (9,2) {v7};
    \node[circle,draw] (j) at (10,3) {v8};
    \foreach \from/\to in {a/b, a/c, b/d, c/d, d/e, e/f, f/g, g/h, g/i, i/j, h/j}
    \draw (\from) -- (\to);
    \draw[dashed, red] (-1,3) .. controls (1,0) and (4,0) .. (5,3) 
    .. controls (2,6) and (0,6) .. (-1,3) ;
    \draw[dashed, blue] (11,3) .. controls (9,0) and (6,0) .. (3,3) 
    .. controls (7,6) and (10,6) .. (11,3) ;
    
    \draw[dashed, red] (0,-1) -- (1,-1);
\node[text width=6cm, anchor=west, right] at (2,-1)
    {neighbour set of s1};
    \draw[dashed, blue] (0,-2) -- (1,-2);
\node[text width=6cm, anchor=west, right] at (2,-2)
    {neighbour set of s2};

  \end{tikzpicture}
  \caption{An instance of SLRL, showing the neighbour sets of the servers s1 and s2}
\end{figure}
...
\end{figure}
\noindent
In the following we prove that 3SAT has a solution if and only if SLRL has a solution. 
First, it will be shown that if 3SAT has a solution, also SLRL has a solution.
\medskip
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[2]
    ...
    maximum size of the neighbour sets of
...
\If{(curS.lastC $\leq$ c) $\wedge$ (curS.r $<$ bestS.r) }
...
\EndIf
\EndIf  
\If{($\neg$bestS.solved) $\wedge$ (bestS.lastC$>$curS.lastC) }
